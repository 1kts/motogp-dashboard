...
const MotoGPDashboard = () => {
  ...
  const loadData = async () => {
    try {
      const csvContent = await window.fs.readFile('RidersSummary.csv', { encoding: 'utf8' });
  ...
  // Preparar dades per al bump chart simulat
  const generateBumpChartData = () => {
    ...
    // Simulem l'evolució cursa a cursa
    const races = 20; // Assumim 20 curses
    const bumpData = [];
    
    for (let race = 1; race <= races; race++) {
      const raceData = { race };
      sortedRiders.forEach(rider => {
        // Calculem punts acumulats de manera progressiva
        const progressRatio = race / races;
        const pointsAtRace = Math.round(rider.points * progressRatio);
        
        // Afegim variabilitat per simular alts i baixos
        const variation = rider.wins > 5 ? 0.15 : 0.05; // Més variabilitat per pilots amb moltes victòries
          ...
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
            ...
            <div className="mb-4 p-4 bg-yellow-900/20 rounded-lg border border-yellow-600/30">
              <p className="text-sm text-yellow-200">
                ...
                Per un bump chart real necessitaríem dades cursa per cursa.
              </p>
            </div>
